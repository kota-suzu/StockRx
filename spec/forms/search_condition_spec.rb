...
RSpec.describe SearchCondition, type: :model do
  ...
  describe 'validations' do
    describe 'field' do
      it 'validates presence' do
        ...
        expect(condition.errors[:field]).to include(I18n.t('errors.messages.blank'))
      end

      it 'validates inclusion in ALLOWED_SEARCH_FIELDS' do
        condition.field = 'invalid_field'
        expect(condition).not_to be_valid
        expect(condition.errors[:field]).to include(I18n.t('errors.messages.inclusion'))
      end
    ...
    describe 'operator' do
      it 'validates inclusion in OPERATORS keys' do
        ...
        expect(condition).not_to be_valid
        expect(condition.errors[:operator]).to include(I18n.t('errors.messages.inclusion'))
      end
    ...
    describe 'logic_type' do
      it 'validates inclusion in LOGIC_TYPES' do
        ...
        expect(condition).not_to be_valid
        expect(condition.errors[:logic_type]).to include(I18n.t('errors.messages.inclusion'))
      end
    ...
    describe 'data_type' do
      it 'validates inclusion in DATA_TYPES' do
        ...
        expect(condition).not_to be_valid
        expect(condition.errors[:data_type]).to include(I18n.t('errors.messages.inclusion'))
      end

      it 'accepts valid data types' do
        valid_data_type_combinations = {
          'string' => { field: 'name', value: 'test' },
          'integer' => { field: 'quantity', value: '123' },
          'decimal' => { field: 'price', value: '123.45' },
          'date' => { field: 'created_at', value: '2023-12-31' },
          'boolean' => { field: 'status', value: 'true' }
        }

        valid_data_type_combinations.each do |data_type, config|
          condition.data_type = data_type
          condition.field = config[:field]
          condition.operator = 'equals'
          condition.value = config[:value]
          expect(condition).to be_valid, "Expected #{data_type} with value '#{config[:value]}' to be valid"
        end
      end
    end
    ...
    describe 'value presence for operator' do
      it 'requires value for most operators' do
        ...
        condition.value = ''
        expect(condition).not_to be_valid
        expect(condition.errors[:value]).to include(I18n.t('errors.messages.blank'))
      end
    ...
    describe 'value type consistency' do
      context 'with integer data type' do
        ...
        it 'rejects non-integer values' do
          condition.value = 'abc'
          expect(condition).not_to be_valid
          expect(condition.errors[:value]).to include(I18n.t('errors.messages.invalid'))
        end
      end
  ...
  describe '#description' do
    ...
    it 'generates human readable description' do
      ...
      expect(result).to include('Name') # humanize capitalizes first letter
